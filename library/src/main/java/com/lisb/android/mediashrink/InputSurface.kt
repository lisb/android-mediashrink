 ...
 *
 * @param surface The Surface that the MediaCodec receives buffers from.
 */
internal class InputSurface(private var surface: Surface?) {
     ...
     */
    init {
        if (surface == null) throw NullPointerException("surface is null")
        eglSetup()
    }
...
// to be able to tell if the frame is reasonable.
        ...
        if (mEGLSurface == null) throw RuntimeException("surface was null")
    }
     ...
     */
    fun release() {
        if (EGL14.eglGetCurrentContext() == mEGLContext) { // Clear the current context and surface to ensure they are discarded immediately.
